...
expect_regex "$WORK_STYLESHEET" "@media \\(max-width: 959px\\) \\{[[:space:][:print:]]*\\.policy-page \\.work-article-meta,[[:space:][:print:]]*\\.policy-page \\.work-article-body,[[:space:][:print:]]*\\.case-study-page \\.work-article-meta,[[:space:][:print:]]*\\.case-study-page \\.work-article-body \\{[[:space:][:print:]]*grid-column: 1;" "Mobile-shell case-study article columns should collapse to the first grid column."
expect_regex "$WORK_STYLESHEET" "@media \\(max-width: 959px\\) \\{[[:space:][:print:]]*\\.case-study-block-two-up \\.case-study-block-inner,[[:space:][:print:]]*\\.case-study-block-two-up\\.case-study-block-two-up-asymmetric \\.case-study-block-inner \\{[[:space:][:print:]]*display: flex;[[:space:][:print:]]*flex-direction: column;" "Mobile-shell two-up blocks should stack vertically."
expect_regex "$WORK_STYLESHEET" "@media \\(max-width: 959px\\) \\{[[:space:][:print:]]*\\.case-study-block-full-media,[[:space:][:print:]]*\\.case-study-block-two-up,[[:space:][:print:]]*\\.case-study-block-two-up\\.case-study-block-two-up-contained,[[:space:][:print:]]*\\.case-study-block-aside-media,[[:space:][:print:]]*\\.case-study-block-carousel \\{[[:space:][:print:]]*width: 100%;[[:space:][:print:]]*margin-left: 0;" "Mobile-shell contained two-up blocks should clear desktop width and offset."
...
expect_regex "$WORK_STYLESHEET" "@media \\(max-width: 700px\\) \\{[[:space:][:print:]]*\\.case-study-block-meta \\.case-study-block-inner \\{[[:space:][:print:]]*display: flex;[[:space:][:print:]]*flex-direction: column;" "Phone metadata should use a single-column summary stack."
expect_pattern "$WORK_STYLESHEET" "border-bottom: 1px solid var(--line);" "Phone metadata should read as a ruled summary block."
expect_pattern "$WORK_STYLESHEET" ".case-study-block-meta .work-article-meta-card + .work-article-meta-card" "Phone metadata should divide rows between metadata items."
