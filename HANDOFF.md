...
- Added a fixed black viewport backdrop behind both home and work shells and declared the site dark-only at the document/CSS root level so Safari-owned surfaces no longer fall back to gray or white; work pages also now declare a black `theme-color`.
...
- On mobile Safari in both light and dark system appearance, pull-to-refresh on `/`, `/work`, `/work/resy-discovery/`, and `/work/sendmoi/` and confirm the exposed browser-owned surfaces stay black.
