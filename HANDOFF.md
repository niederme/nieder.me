...
- Built and iterated a standalone `/mailmoi/` marketing page with:
  ...
  - concise feature grid using paired light/dark feature images from `assets/images/mailmoi/features/`
...
- Added and refined temporary App Store launch-state treatment:
  ...
  - simplified badge source paths to flat files in `assets/images/mailmoi/app-store-badges/`
- Tuned mobile styling on `/mailmoi/`:
...
- Added/updated MailMoi utility pages and homepage references:
...
- Run final visual QA on `/mailmoi/` in iPhone Safari (annotation placement and badge readability) after cache-clear/hard refresh.
...
3. `git pull --ff-only`
4. `make`
5. Validate `/mailmoi/` in desktop and iPhone Safari, then wire live App Store links when available
