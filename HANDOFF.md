...
- Added a dedicated width-driven breakpoint in `assets/css/styles.css` for `431px` to `950px`, so this wide-phone / narrow-tablet range uses a compact static homepage composition instead of the stretched tablet-ish layout.
...
- `820x900` viewport (WebKit)
- `iPhone 14` landscape (`844x390`, WebKit)
- `iPad Mini` portrait (`744x1133`, WebKit)
- Confirmed the homepage now transitions directly from the small-phone layout into the same compact two-column/static composition across the full `431px` to `950px` width band.
